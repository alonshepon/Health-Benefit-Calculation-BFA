library(openxlsx)
library(tidyverse)
setwd("d:/Dropbox (Personal)/Dropbox (Personal)/Nutrient Gaps/Health Benefits calculations/Data/IHME/excels/meat")
my_files<-list.files(pattern="*.csv")
nba<-lapply(my_files,function(i){
  x=read.csv(i)
  x$file
  x})
nba_meat<-do.call("rbind.data.frame",nba)
...
nba_meat<-merge(x=nba_meat,y=countries_all_level_concise,by.x="location",by.y="location_id",all.x=TRUE)
...
saveRDS(nba, file = "my_meat_data.rds")
