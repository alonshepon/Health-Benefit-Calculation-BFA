...
fish <- data_orig %>% 
  ...
  mutate(fish_diff_cap=pmin(20, fish_diff))

hist(fish$fish_diff, breaks=seq(0,120,5))
...
# Non-aquatic ASF
asf_non_aqa_groups <- c("Bovine", "Ovine", "Pork", "Eggs", "Poultry", "Butter", "Milk and Dairy")
asf_non_aq <- data_orig %>% 
  # Reduce to 2030
  filter(year==2030 & food %in% asf_non_aqa_groups) %>% 
  # Simplify
  select(iso3, country, food, value_lo, value_hi) %>% 
  # Summarize
  group_by(iso3, country) %>% 
  summarize(value_lo=sum(value_lo),
            value_hi=sum(value_hi)) %>% 
  ungroup() %>% 
  mutate(food="Non-aquatic ASF", 
         value_diff=value_hi-value_lo,
         value_diff_perc=(value_hi-value_lo)/value_lo*100) %>% 
  # Rename
  rename(asf_nonaq_lo=value_lo, 
         asf_nonaq_hi=value_hi, 
         asf_nonaq_diff=value_diff, 
         asf_nonaq_pdiff=value_diff_perc)
...
data <- fish %>% 
  ...
  # Add all non-aqautic ASF
  left_join(asf_non_aq %>% select(-c(country, food)), by="iso3") %>%
  # Quantify directions
...
g1 <- ggplot(data_sf) +
  ...
  # Legend
  scale_fill_gradient2(name="Difference (g/p/d)\nin 2030 intake\n(high vs. base)",
                       breaks=seq(5,20,5), labels=c("5", "10", "15", "≥20"),
                       midpoint=0, low="darkred", high="navy", mid="white", na.value = "grey80") +
  guides(fill = guide_colorbar(ticks.colour = "black", frame.colour = "black",
                               barwidth = 0.7, barheight = 2.5)) +
  # Crop out Antarctica
  coord_sf(y=c(-55, NA)) +
  # Theme
  theme_bw() + base_theme +
  theme(legend.position = c(0.11,0.36))
g1
