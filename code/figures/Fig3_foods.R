...
all_asf_nonaq <- data_orig %>% 
  # Reduce to 2030
  filter(year==2030 & food %in% c("Bovine", "Ovine", "Pork", "Eggs", "Poultry", "Butter", "Milk and Dairy")) %>% 
  # Simplify
  select(iso3, country, food, value_lo, value_hi) %>% 
  # Summarize
  group_by(iso3, country) %>% 
  ...
  mutate(food="Non-aquatic ASF", 
         ...
         value_diff_perc=(value_hi-value_lo)/value_lo*100) %>% 
  # Rename
  rename(nonaq_asf_lo=value_lo, 
         nonaq_asf_hi=value_hi, 
         nonaq_asf_diff=value_diff, 
         nonaq_asf_pdiff=value_diff_perc)
...
g1 <- ggplot(data_sf) +
  ...
  # Labels
  labs(title="A. Aquatic animal-source food consumption") +
  # Legend
...
# Plot data
g6 <- ggplot(data_sf) +
  geom_sf(mapping=aes(fill=nonaq_asf_pdiff), lwd=0.1, color="grey30") +
  # Plot small places
  geom_sf(data=sdata_pt %>% filter(!is.na(nonaq_asf_pdiff)), mapping=aes(fill=nonaq_asf_pdiff), shape=21, size=0.9, stroke=0.3) +
  # Plot French Guiana
  geom_sf(data=fguiana, lwd=0.1, color="grey30", fill="grey80") +
  # Labels
...
ggsave(g, filename=file.path(plotdir, figname), 
       width=6.5, height=4.5, units="in", dpi=600, cairo_pdf)
