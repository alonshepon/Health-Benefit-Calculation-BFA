...
data_orig <- readRDS(file.path(outputdir, "COSIMO_nutrient_by_scenario_cntry_with_dissagg.Rds")) %>% 
  ...
  # Convert to per year
  mutate(intake=intake*365,
         intake_orig=intake*365,
         intake_diff=intake_diff*365,
         nutrient_units=gsub("/d", "/yr", nutrient_units))
...
plot_boxplot <- function(nutrient){
  ...
  # Plot data
  g <- ggplot(sdata, aes(y=intake_diff, x=scenario)) +
...
# Export plot
ggsave(g, filename=file.path(plotdir, "Fig3_nutrients_yr.png"), 
